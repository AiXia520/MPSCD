...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x, le: bool = False, ft: bool = False):
        ...
        if ft:
            ...
            if le:
                le, x = self.classifier(x, le=True)
                return features, le, x, feats
            else:
                x = self.classifier(x)
                return features, x
        else:
...
def mobilenet_v2(num_classes, pretrained=True):
    ...
    add_ml_decoder_head(model, num_classes=num_classes, num_of_groups=num_classes, decoder_embedding=768)
    return model

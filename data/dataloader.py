...
def voc(cfg, data_root):
    ...
    train_dataset = VocDataset(cfg, train_img_prefix, train_ann_file, class_name, img_size=cfg.img_size, train_mode=True, inp_name=inp_path)
    train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True,
                              num_workers=8, drop_last=True, pin_memory=True)
    train_loader.num_classes = len(train_dataset.CLASSES)
    print(">>> Train Dataloader Built!")

    test_dataset = VocDataset(cfg, test_img_prefix, test_ann_file, class_name, img_size=cfg.img_size, train_mode=False, inp_name=inp_path)
...
def coco(cfg, data_root):
    ...
    train_dataset = CocoDataset(cfg, train_img_prefix, train_ann_file, class_name, img_size=cfg.img_size, train_mode=True, inp_name=inp_path)
    train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True,
                              num_workers=8, drop_last=True)
    train_loader.num_classes = len(train_dataset.CLASSES)
    print(">>> Train Dataloader Built!")

    test_dataset = CocoDataset(cfg, test_img_prefix, test_ann_file, class_name, img_size=cfg.img_size, train_mode=False, inp_name=inp_path)
    test_loader = DataLoader(test_dataset, batch_size=cfg.batch_size, shuffle=True, num_workers=8)
    test_loader.num_classes = len(test_dataset.CLASSES)
    print(">>> Test Dataloader Built!")

...
def get_args():
    ...
    parser.add_argument("--cfg_file", default="./configs/voc", type=str, help="path of cfg file")
    parser.add_argument("--data_root", default="./dataset/", type=str, help="path of data files")
    parser.add_argument("--evaluate", action='store_true', default=False, help="path of data files")
...
def main(args):
    ...
    if not cfg.teacher_pretrained:
        ...
        for epoch in range(cfg.max_epoch_t):
            if epoch >= cfg.stop_epoch_t:
                break
            train(cfg, epoch, train_loader, learner_t)
            AP, mAP, of1, cf1 = evaluate(cfg, test_loader, model_teacher)
    ...
    _, mAP_t, of1_t, cf1_t = evaluate(cfg, test_loader, model_teacher)
    evaluate(cfg, test_loader, model_student)
    print("Finished!\n")

    # student model training

    if args.evaluate:
        print("After distillation, evaluate student model:")

        #baseline
        # model_teacher.load_state_dict(torch.load("pretrained_models/model_teacher_%s_%s_%d.pth"
        #                                          % (cfg.model_t, cfg.dataset, cfg.img_size)))
        # evaluate(cfg, test_loader, model_teacher)
        # print("Finished!\n")

        # model_student.load_state_dict(torch.load("pretrained_models/model_student_%s_%s_%s_%d.pth"
        #                                          % (cfg.model_t, cfg.model_s, cfg.dataset, cfg.img_size)))

        model_student.load_state_dict(torch.load("pretrained_models/model_student_%s_%s_%d.pth"
                                                 % (cfg.model_s, cfg.dataset, cfg.img_size)))
        ...
        learner_s = Learner_KD(cfg, model_teacher, model_student, criterion_s, criterion_t2s, optimizer_s, scheduler_s)
        ...
        for epoch in range(cfg.max_epoch_s):
            if epoch >= cfg.stop_epoch_s:
                break
            train(cfg, epoch, train_loader, learner_s)
            AP, mAP, of1, cf1 = evaluate(cfg, test_loader, model_student)
            ...
            if mAP >= student_mAP:
                student_mAP = mAP
                torch.save(model_student.state_dict(), "student_models/model_student_%s_%s_%s_%d.pth"
                           % (cfg.model_t, cfg.model_s, cfg.dataset, cfg.img_size))


        model_student.eval()
        print("Finally, evaluate student model:")
        model_student.load_state_dict(torch.load("student_models/model_student_%s_%s_%s_%d.pth"
                                                 % (cfg.model_t, cfg.model_s, cfg.dataset, cfg.img_size)))
